...
async def _latest_fitness_from_wellness(
    client: ICUClient,
) -> tuple[float | None, float | None, float | None, float | None]:
    """Read CTL/ATL/TSB/ramp-rate from the most recent wellness row.

    intervals.icu carries these on the daily wellness records even when the
    athlete-profile fitness object is empty, so this is the fallback source for
    ``get_fitness_summary``. Returns ``(ctl, atl, tsb, ramp_rate)``; any element
    may be ``None`` if no recent row has the data.
    """
    newest = datetime.now()
    oldest = newest - timedelta(days=30)
    records, _dropped = await client.get_wellness(
    ...
    for record in records:
        if record.ctl is not None or record.atl is not None:
            ...
            return record.ctl, record.atl, tsb, record.ramp_rate
...
async def get_fitness_summary(
    ...
    try:
        async with ICUClient(config) as client:
            ...
            # intervals.icu sometimes leaves the profile fitness object empty even
            # though the daily wellness rows carry CTL/ATL — fall back to those.
            if ctl is None and atl is None:
                ctl, atl, tsb, ramp_rate = await _latest_fitness_from_wellness(client)
                source = "wellness"
            ...
            # Derive TSB (Fitness - Fatigue) if the source didn't provide it.
